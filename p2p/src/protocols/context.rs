    ...
    for (i, &key) in keys.iter().enumerate() {
        ...
        if is_last {
            if let Some(obj) = current.as_object_mut() {
                ...
                return Err(AviP2pError::InvalidPath(
            ...
            current = current.get_mut(key).ok_or_else(|| {
                AviP2pError::InvalidPath(format!("Failed to navigate to key: {}", key))
            })?;
        }
    }

    Err(AviP2pError::InvalidPath(
        "Unexpected end of path".to_string(),
    ))
}
