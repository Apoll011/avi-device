...
pub async fn setup_dispatcher(handle: AviP2pHandle) -> StreamDispatcher {
    let dispatcher = StreamDispatcher::new(handle);

    // Registra handlers para diferentes tipos de stream
    dispatcher.register_handler("audio".to_string(), AudioHandlerFactory).await;
    dispatcher.register_handler("log".to_string(), LogHandlerFactory).await;
    // dispatcher.register_handler("video".to_string(), VideoHandlerFactory).await;
    // dispatcher.register_handler("screen-share".to_string(), ScreenShareHandlerFactory).await;

    dispatcher
}

pub async fn process_event(dispatcher: &StreamDispatcher, event: AviEvent) {
    match event {
        AviEvent::StreamRequested { from, stream_id, reason } => {
            if let Err(e) = dispatcher.handle_stream_requested(from, stream_id, reason).await {
                eprintln!("Error handling stream request: {}", e);
            }
        }

        AviEvent::StreamAccepted { peer_id, stream_id } => {
            if let Err(e) = dispatcher.handle_stream_accepted(peer_id, stream_id).await {
                eprintln!("Error handling stream accepted: {}", e);
            }
        }

        AviEvent::StreamData { from, stream_id, data } => {
            if let Err(e) = dispatcher.handle_stream_data(from, stream_id, data).await {
                eprintln!("Error handling stream data: {}", e);
            }
        }

        AviEvent::StreamClosed { peer_id, stream_id, reason } => {
            if let Err(e) = dispatcher.handle_stream_closed(peer_id, stream_id, reason).await {
